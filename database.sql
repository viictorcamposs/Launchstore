...
-- 2. Criar uma Regra que roda todas as vezes que for solicitado o DELETE;
CREATE OR REPLACE RULE delete_product AS 
ON DELETE TO products DO INSTEAD
UPDATE products 
SET deleted_at = now()
WHERE products.id = old.id;
-- 3. Criar uma VIEW que puxa somente os dados que estão ativos.
CREATE VIEW products_without_deleted AS
SELECT * FROM products WHERE deleted_at IS NULL;
-- 4. Renomear VIEW e table products para diminuir impacto no código
